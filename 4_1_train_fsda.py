"""
This file is used for training model-FS or model-FS-A, making predictions on the test set,
and writing the predictions to csv files for submission.

By default this script runs model-FS. Consult to the readme.txt for the changes that need to be applied to run
model-FS-A.
"""
    ...
    if len(test_matrix) > 0:
        ...
        for f in range(len(test_matrix)):
            for c in range(num_of_classes):
                submission_id.append(test_matrix['FileName'].values[f] + '.wav_classnumber_' + str(c + 1))
...
def get_nips4b_test_data(load_training_data_):
    """
    Get the test data information required for classification
    :return:
    """

    if load_training_data_:
        pkl_file = open(variable_save_directory + 'test_.pkl', 'rb')
        test_ = pickle.load(pkl_file)
        pkl_file.close()
        return test_
    ...
    output = open(variable_save_directory + 'test_.pkl', 'wb')
    pickle.dump(test_, output)
    output.close()

    return test_


def get_training_data_and_properties(load_training_data_):

    # Save the training design matrix and feature ids to disk to save time in future runs
    if load_training_data_ == 0:
        ...
        num_of_train_files_, num_of_classes_, feature_ids_, num_of_file_and_segment_probs_, train_ = get_nips4b_train_data()

        # Save to file
        output = open(variable_save_directory + 'train_' + '.pkl', 'wb')
        pickle.dump(train_, output)
        output.close()
        output = open(variable_save_directory + 'feature_ids' + '.pkl', 'wb')
        pickle.dump(feature_ids_, output)
        output.close()

    # Load training design matrix and feature ids to save time
    else:
        pkl_file = open(variable_save_directory + 'train_.pkl', 'rb')
        train_ = pickle.load(pkl_file)
        pkl_file.close()
        pkl_file = open(variable_save_directory + 'feature_ids.pkl', 'rb')
        feature_ids_ = pickle.load(pkl_file)
        pkl_file.close()
        num_of_train_files_ = 687
        num_of_classes_ = 88
        num_of_file_and_segment_probs_ = 81

    return train_, feature_ids_, num_of_train_files_, num_of_classes_, num_of_file_and_segment_probs_


def get_feature_ids_associated_with_bird_classes(load_training_data_):

    # If saved to disk already, load the feature_ids_associated_with_bird_classes to save time
    if load_training_data_:
        pkl_file = open(variable_save_directory + 'feature_ids_associated_with_bird_classes.pkl', 'rb')
        feature_ids_associated_with_bird_classes_ = pickle.load(pkl_file)
        pkl_file.close()
        return feature_ids_associated_with_bird_classes_
    ...
    for bird in range(num_of_classes):
        ...
        for ti in list_of_train_files_per_bird_class[bird]:
            ...
            np_array = np.arange(segment_offsets_per_train_file[ti],
                                 segment_offsets_per_train_file[ti] + num_of_segments_per_train_file[ti],
                                 dtype=np.int32)
        ...
        feature_ids_associated_with_bird_classes_[bird] = temp_list

        print('NumOfFeaturesForSoundClass', bird, ':',
              len(feature_ids_associated_with_bird_classes_[bird]), '+', num_of_file_and_segment_probs)

    # Save the feature ids associated with bird classes to disk for future use to save time
    output = open(variable_save_directory + 'feature_ids_associated_with_bird_classes.pkl', 'wb')
    pickle.dump(feature_ids_associated_with_bird_classes_, output)
    output.close()

    return feature_ids_associated_with_bird_classes_


def get_list_of_train_files_per_bird_class(load_training_data_):

    # Load the list of train files per bird class to save time
    if load_training_data_:
        pkl_file = open(variable_save_directory + 'list_of_train_files_per_bird_class.pkl', 'rb')
        list_of_train_files_per_bird_class_ = pickle.load(pkl_file)
        pkl_file.close()
    else:
        # Get training set metadata
        metadata = get_nips4b_metadata()
        list_of_train_files_per_bird_class_ = metadata[4]
        output = open(variable_save_directory + 'list_of_train_files_per_bird_class.pkl', 'wb')
        pickle.dump(list_of_train_files_per_bird_class_, output)
        output.close()

    return list_of_train_files_per_bird_class_


def train(num_of_cv_folds_, num_of_estimators_, max_features_, min_sample_split_, load_training_data_,
          data_aug_percentage_, important_feature_percentage_):
    global train_run_id
    global targets_predicted_per_bird_4_test_files
    global prediction_matrix_test
    global load_training_data
    ...
    print('Cvs, MaxNumEstimators, MaxFeaturesForSplit, MinSampleSplit: , DataAugPercentage, ImportantFeaturePercentage')
    ...
    # If this flag is on, load and use fixed CV indices to eliminate the effect of
    # training with different validation sets
    if use_fixed_cv_indices:
        if num_of_cv_folds_ == 12:
            pkl_file = open(variable_save_directory + 'cv_train_indices_12.pkl', 'rb')
            cv_train_indices = pickle.load(pkl_file)
            pkl_file.close()

        else:
            pkl_file = open(variable_save_directory + 'cv_train_indices_14.pkl', 'rb')
            cv_train_indices = pickle.load(pkl_file)
            pkl_file.close()

    # Else, generate new CV indices for each training run
    else:
        ...
        cv_train_indices = np.random.randint(0, num_of_cv_folds_, len(train_matrix.index))

        # output = open(variable_save_directory + 'cv_train_indices' + '_' + str(num_of_cv_folds_) + '.pkl', 'wb')
        # pickle.dump(cv_train_indices, output)
        # output.close()
        # load_training_data = 1

    train_matrix['cv'] = cv_train_indices
    targets_true = []
    targets_predicted = []
    ...
    if len(test_matrix) > 0:
        ...
        prediction_matrix_test = np.empty((num_of_cv_folds_, num_of_classes, len(test_matrix)), dtype=np.float64)
        for bird_ in range(num_of_classes):
            targets_predicted_per_bird_4_test_files.append(np.zeros(len(test_matrix)))
    ...
    for c in range(num_of_cv_folds_):
        ...
        training_set = train_matrix[train_matrix.cv != c]
        validation_set = train_matrix[train_matrix.cv == c]
        validation_set_indices = validation_set.index.tolist()

        # ================ For debugging
        # print(list(training_set.columns))
        # print("Training set")
        # print(training_set)  # 625 (number of training samples for this cv) * 10220
        # print(training_set[0])
        # print(training_set['FileName'])
        ...
        for bird_ in range(num_of_classes):
            ...
            if len(test_matrix) > 0:
                test_set_features = test_matrix[current_feature_ids]

            # ====== Select most important features
            # Import most important features
            pkl_file_ = open(feature_importance_directory + "class_" + str(bird_) + ".pkl", 'rb')
            fi_sorted = pickle.load(pkl_file_)
            pkl_file_.close()
            most_important_features = fi_sorted['Feature Number'].tolist()
            most_important_features = \
                most_important_features[:math.ceil(len(most_important_features)*important_feature_percentage_)]

            # Get rid of all features except the most important ones
            # (select the given percentage of most important features: 25%, 50%, 75%, etc)
            training_set_features = training_set_features[most_important_features]
            validation_set_features = validation_set_features[most_important_features]
            test_set_features = test_set_features[most_important_features]

            # ========= For debugging
            # 625 (number of training samples for this cv) * 229 (number of features for this class)
            # 62 (number of validation samples for this cv) * 229 (number of features for this class)
            # print("Training set features")
            # print(training_set_features)
            # print("Validation set features")
            # print(validation_set_features)
            # print("Test set features")
            # print(test_set_features)
            # ========

            # DATA AUGMENTATION
            # Here we add a number of meaningless predictor variables to the design matrices
            # The number of variables we add are proportional to the current number of variables in
            # the current design matrices (e.g. 10% of total number of features, 25% of total number of features, etc)
            if data_aug_percentage_ > 0:
                num_training_samples, num_features = training_set_features.shape
                num_validation_samples = validation_set_features.shape[0]
                num_test_samples = test_set_features.shape[0]
                num_aug_features = math.ceil(num_features * data_aug_percentage_)
                train_aug = np.random.uniform(0, 1, (num_training_samples, num_aug_features))
                val_aug = np.random.uniform(0, 1, (num_validation_samples, num_aug_features))
                test_aug = np.random.uniform(0, 1, (num_test_samples, num_aug_features))
                aug_columns = ['Faug_' + str(x) for x in range(num_aug_features)]

                train_aug = pd.DataFrame(train_aug, columns=aug_columns)
                val_aug = pd.DataFrame(val_aug, columns=aug_columns)
                test_aug = pd.DataFrame(test_aug, columns=aug_columns)
                train_aug['FileName'] = training_set_features.index
                val_aug['FileName'] = validation_set_features.index
                test_aug['FileName'] = test_set_features.index

                training_set_features_aug = pd.merge(left=training_set_features,
                                                     right=train_aug, left_index=True, right_on='FileName')
                training_set_features_aug.drop('FileName', axis=1, inplace=True)
                validation_set_features_aug = pd.merge(left=validation_set_features,
                                                       right=val_aug, left_index=True, right_on='FileName')
                validation_set_features_aug.drop('FileName', axis=1, inplace=True)
                test_set_features_aug = pd.merge(left=test_set_features,
                                                 right=test_aug, left_index=True, right_on='FileName')
                test_set_features_aug.drop('FileName', axis=1, inplace=True)
            else:
                training_set_features_aug = training_set_features
                validation_set_features_aug = validation_set_features
                test_set_features_aug = test_set_features

            # ========= For debugging
            # print(training_set_features_aug)
            # print(validation_set_features_aug)
            # print(test_set_features_aug)
            # ========================
            ...
            num_of_estimators_current_bird = int(2 * len(most_important_features))
            ...
            classifier = ExtraTreesRegressor(n_estimators=num_of_estimators_current_bird, max_features=max_features_,
                                             min_samples_split=min_sample_split_,
                                             random_state=(train_run_id + bird_) * 100, verbose=0)
            classifier.fit(training_set_features_aug, training_set_targets)

            """
            # Get feature importances and write to file
            feature_importances = classifier.feature_importances_

            if c == 0:
                with open(current_path + "/feature_importances/" + "class_" + str(bird_) + ".csv", 'a') \
                        as feature_importance_file:
                    wr = csv.writer(feature_importance_file)
                    wr.writerows([current_feature_ids])
            with open(current_path + "/feature_importances/" + "class_" + str(bird_) + ".csv", 'a') \
                    as feature_importance_file:
                wr = csv.writer(feature_importance_file)
                wr.writerows([feature_importances])
            """

            # Make predictions on the validation and test sets
            # validation_set_predictions = classifier.predict(validation_set_features)
            validation_set_predictions = classifier.predict(validation_set_features_aug)

            if len(test_matrix) > 0:
                # test_set_predictions = classifier.predict(test_set_features)
                test_set_predictions = classifier.predict(test_set_features_aug)
    ...
    log('DataAugPercentage:                 ' + str(data_aug_percentage_))
    log('ImportantFeaturePercentage:        ' + str(important_feature_percentage_))
    log('')
...
variable_save_directory = current_path + '/outputs/variables/'
feature_importance_directory = current_path + '/feature_importances/'

if not os.path.exists(variable_save_directory):
    os.makedirs(variable_save_directory)
if not os.path.exists(feature_importance_directory):
    os.makedirs(feature_importance_directory)

# Option variables
include_file_and_segment_properties = 1
use_fixed_cv_indices = 0
...
# data_aug_percentage_list = [0.1, 0.25]
data_aug_percentage_list = [0]
most_important_features_percentage_list = [0.25, 0.50, 0.75]

# Get training related data
train_matrix, feature_ids, num_of_train_files, num_of_classes, \
    num_of_file_and_segment_probs = get_training_data_and_properties(load_training_data)

print('NumOfTrainFiles:', num_of_train_files, '  NumOfClasses:', num_of_classes, 'NumOfFeatures:', len(feature_ids))
print("\n")

list_of_train_files_per_bird_class = get_list_of_train_files_per_bird_class(load_training_data)
feature_ids_associated_with_bird_classes = get_feature_ids_associated_with_bird_classes(load_training_data)

# Get testing related data
num_of_test_files = 1000
test_matrix = pd.DataFrame()
test_matrix = get_nips4b_test_data(load_testing_data)
        ...
        for num_of_estimators in num_of_estimators_list:
            for max_features in max_features_list:
                for min_sample_split in min_sample_split_list:
                    for data_aug_percentage in data_aug_percentage_list:
                        for important_feature_percentage in most_important_features_percentage_list:
                            train(num_of_cv_folds, num_of_estimators, max_features, min_sample_split,
                                  load_training_data, data_aug_percentage, important_feature_percentage)
